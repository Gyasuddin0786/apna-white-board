...
import { Avatar, IconButton, Tooltip, Menu, MenuItem, Divider } from '@mui/material';
import {
  Add, Delete, OpenInNew, Logout, DarkMode, LightMode,
  Search, MoreVert, DriveFileRenameOutline, People,
} from '@mui/icons-material';

const TEMPLATES = [
  { name: 'Blank Board', icon: '⬜', elements: [] },
  {
    name: 'Flowchart', icon: '🔄', elements: [
  ...
  {
    name: 'Wireframe', icon: '🖥️', elements: [
...
const RenameInput = ({ value, onSave, onCancel }) => {
  ...
  return (
    <input
      ...
      className="w-full text-sm font-semibold px-1 py-0.5 rounded border border-indigo-400 outline-none bg-transparent"
    />
  );
};
...
const DashboardPage = () => {
  ...
  const createBoard = async (template) => {
    try {
      const { data } = await api.post('/boards', {
        title: template.name === 'Blank Board' ? 'Untitled Board' : template.name,
  ...
  const openBoard = (id) => window.open(`/board/${id}`, '_blank');
  ...
  const renameBoard = async (id, newTitle) => {
    ...
    if (!trimmed) return;
    try {
      await api.put(`/boards/${id}`, { title: trimmed });
      setBoards((prev) => prev.map((b) => b._id === id ? { ...b, title: trimmed } : b));
    } catch {
      toast.error('Failed to rename');
    }
  };

  const isOwner = (board) => board.owner._id === user?.id || board.owner._id === user?._id;

  const availableYears = useMemo(() => {
    const years = [...new Set(boards.map((b) => new Date(b.updatedAt).getFullYear()))];
  ...
  const { myBoards, sharedBoards } = useMemo(() => {
    const filtered = boards.filter((b) => {
      ...
      const matchMonth = filterMonth === '' || d.getMonth() === parseInt(filterMonth);
      const matchYear = filterYear === '' || d.getFullYear() === parseInt(filterYear);
      return matchSearch && matchMonth && matchYear;
    });
    return {
      myBoards: filtered.filter((b) => isOwner(b)),
      sharedBoards: filtered.filter((b) => !isOwner(b)),
    };
  }, [boards, search, filterMonth, filterYear, user]);
  ...
  const BoardCard = ({ board }) => {
    ...
    const collabCount = board.collaborators?.length || 0;

    return (
      <div
        onClick={() => renamingId !== board._id && openBoard(board._id)}
        className={`group relative border rounded-2xl overflow-hidden cursor-pointer hover:shadow-lg transition-all ${cardBg} ${darkMode ? 'hover:border-indigo-700' : 'hover:border-indigo-300'}`}
      >
        {/* Preview */}
        <div className={`h-28 flex items-center justify-center relative ${darkMode ? 'bg-gray-800' : 'bg-gradient-to-br from-indigo-50 to-purple-50'}`}>
        ...
        <div className="p-3">
          {/* Title — inline rename for owned boards */}
          ...
          <p className={`text-xs mt-0.5 ${subText}`}>{formatDate(board.updatedAt)}</p>
          ...
          <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
            ...
            {owned && collabCount > 0 && (
              <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium flex items-center gap-0.5 ${darkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-500'}`}>
                <People sx={{ fontSize: 11 }} />{collabCount}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <IconButton
            size="small"
            sx={{ bgcolor: darkMode ? '#1e293b' : 'white', '&:hover': { bgcolor: darkMode ? '#334155' : '#f0f0f0' }, width: 28, height: 28 }}
            onClick={(e) => { e.stopPropagation(); setAnchorEl(e.currentTarget); setMenuBoard(board); }}
          >
  ...
  const SectionGrid = ({ title, items, emptyMsg }) => (
    <div className="mb-10">
      <h2 className={`text-base font-semibold mb-4 flex items-center gap-2 ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>
        {title}
        <span className={`text-xs font-normal ${subText}`}>({items.length})</span>
      </h2>
      {items.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {items.map((board) => <BoardCard key={board._id} board={board} />)}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className={`min-h-screen ${bg}`}>
      {/* Header */}
      <header className={`border-b px-6 py-3.5 flex items-center justify-between sticky top-0 z-30 ${headerBg}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
            <Avatar sx={{ width: 26, height: 26, bgcolor: '#6366f1', fontSize: 12 }}>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {TEMPLATES.map((tpl) => (
              <button key={tpl.name} onClick={() => createBoard(tpl)}
                className={`group flex flex-col items-center justify-center h-24 border-2 border-dashed rounded-2xl transition-all ${darkMode ? 'border-gray-700 hover:border-indigo-500 hover:bg-indigo-950' : 'border-gray-200 hover:border-indigo-400 hover:bg-indigo-50'}`}>
        ...
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => (
              <div key={i} className={`h-40 rounded-2xl animate-pulse ${darkMode ? 'bg-gray-800' : 'bg-gray-200'}`} />
            ))}
          </div>
        ) : (
          <>
      ...
      <Menu
        ...
        PaperProps={{ sx: { bgcolor: darkMode ? '#1e293b' : 'white', border: `1px solid ${darkMode ? '#334155' : '#e2e8f0'}`, borderRadius: 2, minWidth: 170 } }}
      >
        <MenuItem onClick={() => { openBoard(menuBoard?._id); setAnchorEl(null); }}
          sx={{ fontSize: 13, color: darkMode ? '#e2e8f0' : '#374151', gap: 1.5 }}>
          <OpenInNew sx={{ fontSize: 16 }} /> Open in New Tab
        </MenuItem>

        {/* Rename — only owner */}
        {menuBoard && isOwner(menuBoard) && (
          <MenuItem onClick={() => { setRenamingId(menuBoard._id); setAnchorEl(null); }}
            sx={{ fontSize: 13, color: darkMode ? '#e2e8f0' : '#374151', gap: 1.5 }}>
            <DriveFileRenameOutline sx={{ fontSize: 16 }} /> Rename
          </MenuItem>
        )}
        ...
        {menuBoard && isOwner(menuBoard) && [
          ...
          <MenuItem key="del" onClick={() => deleteBoard(menuBoard._id)}
            sx={{ fontSize: 13, color: '#ef4444', gap: 1.5 }}>
            <Delete sx={{ fontSize: 16 }} /> Delete Board
          </MenuItem>
        ]}
      </Menu>
    </div>
